...
def intersection_over_union(boxes_preds, boxes_labels, box_format="midpoint"):
    ...
    if box_format == "midpoint":
        box1_x1 = boxes_preds[..., 0:1] - boxes_preds[..., 2:3] / 2#Take all the elements in the 3rd entry in the last axis in all other axices.
...
def non_max_suppression(bboxes, iou_threshold, threshold, box_format="corners"):
    ...
    bboxes = sorted(bboxes, key=lambda x: x[1], reverse=True) #Sorting the boxes with highest prob to lowest prob
    bboxes_after_nms = []
    ...
    while bboxes:
        ...
        bboxes = [
            box
            for box in bboxes
            if box[0] != chosen_box[0]          #If the class of any box is different from the bboxes[0]
            or intersection_over_union(
                torch.tensor(chosen_box[2:]),   #Or If the IOU between the bboxes[0]
                torch.tensor(box[2:]),          #and any other box
                box_format=box_format,
            )
            < iou_threshold                     #is greater than the IOU. 
        ]                                       #Keep only those boxes in bboxes list. Remove all the other.
